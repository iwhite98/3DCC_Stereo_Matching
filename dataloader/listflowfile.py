...
def dataloader(filepath, filepath_aug):
	...
	monkaa_path_aug = filepath_aug + [x for x in image if 'monkaa' in x][0]
	...
	for dd in monkaa_dir:
		...
		for im in os.listdir(monkaa_path_aug+'/'+dd+'/left/'):
			if is_image_file(monkaa_path_aug+'/'+dd+'/left/'+im):
				all_left_aug_img.append(monkaa_path_aug+'/'+dd+'/left/'+im)
	...
	flying_path_aug = filepath_aug + [x for x in image if x == 'frames_cleanpass'][0]
	flying_disp = filepath + [x for x in disp if x == 'frames_disparity'][0]
	flying_dir = flying_path+'/TRAIN/'
	flying_dir_aug = flying_path_aug+'/TRAIN/'
	subdir = ['A','B','C']
	...
	for ss in subdir:
		...
		for ff in flying:
			...
			for im in imm_l:
				...
				if is_image_file(flying_dir_aug+ss+'/'+ff+'/left/'+im):
					all_left_aug_img.append(flying_dir_aug+ss+'/'+ff+'/left/'+im)
				if is_image_file(flying_dir+ss+'/'+ff+'/right/'+im):
					all_right_img.append(flying_dir+ss+'/'+ff+'/right/'+im)
	flying_dir = flying_path+'/TEST/'
	flying_dir_aug = flying_path_aug+'/TEST/'
	...
	for ss in subdir:
		...
		for ff in flying:
			...
			for im in imm_l:
				...
				if is_image_file(flying_dir_aug+ss+'/'+ff+'/left/'+im):
					test_left_aug_img.append(flying_dir_aug+ss+'/'+ff+'/left/'+im)
	...
	driving_dir_aug = filepath_aug + [x for x in image if 'driving' in x][0] + '/'
		...
		for j in subdir2:
			for k in subdir3:
				...
				for im in imm_l:
					...
					if is_image_file(driving_dir_aug+i+'/'+j+'/'+k+'/left/'+im):
						all_left_aug_img.append(driving_dir_aug+i+'/'+j+'/'+k+'/left/'+im)
					if is_image_file(driving_dir+i+'/'+j+'/'+k+'/right/'+im):
						all_right_img.append(driving_dir+i+'/'+j+'/'+k+'/right/'+im)


	return all_left_img, all_right_img, all_left_disp, test_left_img, test_right_img, test_left_disp, all_left_aug_img, test_left_aug_img
